...
- 工具卡片不再共用同一套折叠规则。`read`、`write`、`edit`、`apply_patch`、`todo_write`、`handoff` 现在和 `delete`、各类报告卡一样恒展开：正文本身——文件内容、diff、todo 列表、计划路径——就是卡片的全部内容，因此不再显示 `▸` / `▾` 标记，`Space`、`Enter`、`o` 对它们不生效。`grep`、`glob`、`shell`、`cancel` 和通用工具调用仍可折叠。
- 结束的后台 `shell` 任务改用折叠的 `JOB RESULT` 卡片：默认保留每个 job 的标题行与一行状态，展开后显示完整的命令、状态与输出。运行期状态卡（loop 提示、info 卡、上下文压力提示、`REPLY RESUMED`）同样默认只显示前三行并带 `▸` 标记，不再直接铺开全文。
