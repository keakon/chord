...
- `edit`、`apply_patch` 和 `write` 现在会在写入前清理工具参数中的零宽格式字符与孤立组合符号（零宽空格、零宽不连字、emoji 序列之外的零宽连字、词连接符、流中 BOM、软连字符；以及没有基字符可依附的变音符号，如游离的长音符），泄漏的不可见字节不会再写进文件或破坏字符串匹配。工具结果会报告具体清理了哪些码点（例如 `U+200B×2, U+0304×1`），让模型学会不再输出它们。
...
- `edit`、`apply_patch` 的容错匹配现在也会折叠孤立的变音符号（U+0304 一类）：出现在行首、或紧跟空格/标点/数字之后——这是复制标题（「`### ̄.2.1`」）时 token 化泄漏会产生的形状，真实文件在那个位置不会存在。字母后的变音符号仍然保留，可能是阿拉伯语、天城文、越南语等文字的合法组合，这些文字照常匹配——包括一个字母携带多个变音符号的堆叠序列。
