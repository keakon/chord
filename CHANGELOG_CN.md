...
- 压缩阈值现在可以按模型调，压力提醒线可配置。模型定义或 `model_templates` 条目可带 `compaction` 块（`threshold` 与/或 `reminder`）；未设字段继承全局 `context.compaction`（该模型 `threshold: 0` 只对模型禁用自动压缩；`context.compaction.reminder` 设全局提醒线，`0` 表示按 `min(0.60, threshold×0.90)` 派生）。提醒在 `min(reminder, threshold)`（任一先到）触发——所以 reminder 即使高于 threshold，threshold 越线本身也会触发提醒。自动压缩阈值越线后不再立即压缩：Chord 会开一个 2 个主请求的宽限期，期间模型能看到压力提醒和外化提示，可主动调用 `compact_context` reset 或把状态写入文件；宽限到期（或 oversize 时立即）才启动压缩，model-driven 请求被 skip/失败/取消会提前结束宽限。切换模型会套用该模型的覆盖并开启新的宽限/提醒窗口。详见[上下文压缩](./docs/context-management_CN.md#上下文压缩compaction)。
