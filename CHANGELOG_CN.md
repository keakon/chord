...
- 流式输出中断后（provider 中途推错误事件如 `error` / `response.failed`、传输被截断、读超时），模型已经输出的正文不再被丢弃重生成：它会留在屏幕上，并作为一条被中断的回复存进会话历史，随后这一轮通过正常的 key / 兜底模型轮转重新发起请求、从断开处接着写。自动续写没有轮数上限：只要每轮中断后都还有新的可见正文就继续，直到回复完成、某轮没有产出正文或回合被取消。能从被中断的回复（assistant 结尾）直接续写的模型池不需要任何额外输入；其它模型池会在这条被中断的回复之后追加一条真实的续写消息——同一段文本原样进入本次及之后每一次模型请求，并以 `REPLY RESUMED` 卡片显示在续写正文之前，用户看到的卡片内容就是模型实际收到的内容，恢复会话后该卡片依然可见。明确标识上游/网关流中断的错误事件（`upstream_connection_error`、`upstream stream was interrupted`）属于确定性的上游故障，会在有限次重试轮后停止并明确报告故障，不再无限重试。没有产生正文的中断（只输出了推理、已经开始输出工具调用）仍走原来的静默重试；本身没有可用输出的失败（请求参数错误、上下文超限）仍立即丢弃重来。
