...
- 模型驱动的 checkpoint 不再丢掉当前回合里失败的工具调用。重置会归档整个 head，此前被拒绝的调用（例如 runtime 驳回的 `compact_context` 请求）只留在 checkpoint 内的摘录里：错误卡片从会话中消失，对该代做 fork 时也不再回放这次失败。现在失败的批次会作为真实记录保留在 checkpoint 卡片之后。
