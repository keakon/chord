...
- 被后续编辑或重读标记为陈旧的 read，不再一检测到就改写已发送的 prompt 前缀。这项裁剪只省下很少 token，但改写一条已经进入服务商缓存前缀的消息，会让该位置之后的缓存全部失效、把后面整段对话按未命中输入重新计费——此前即使省下的量远不足以回本也会改写。现在陈旧 read 与其他裁剪一样等待摊还门，只在冷缓存或收益足以回本时落地；已经带 `truncated=stale`/`superseded` 提示的 read 标记保持逐字节稳定，不再重渲染成等价标记。
