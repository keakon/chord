...
- `edit`、`apply_patch` 和 `write` 现在会在写入前清理工具参数中的零宽格式字符（零宽空格、零宽不连字、emoji 序列之外的零宽连字、词连接符、流中 BOM、软连字符），泄漏的不可见字节不会再写进文件或破坏字符串匹配。工具结果会报告具体清理了哪些码点（例如 `U+200B×2`），让模型学会不再输出它们。
- `edit` 失败时，即使目标文本在很大的文件深处，也能定位最近匹配块：错误信息给出文件行号、相似度和具体差异行（比如缺 `)` 或双逗号 `,,`），不再只建议重读。提示还会按位置和码点标出首个差异字符（`first mismatch at rune 4: your line has U+0020, file has U+0078`），并把孤立变体选择符等不可见字符转义成 `\ufe0f` 显示，避免两行看起来相同却匹配失败的不可见差异。此前深处窗口的搜索预算在文件开头附近就耗尽，只能退回泛化提示。
...
- 流式输出中断后（provider 中途推错误事件如 `error` / `response.failed`、传输被截断、读超时），模型已经输出的正文会留在屏幕上，Chord 在后台继续重试该请求：默认 `stream_retry_rounds: 0` 表示按递增退避一直重试直到成功或用户取消，重试一旦开始产出新内容，屏幕上保留的旧正文会立刻被替换，回复不会拼上两次尝试的内容。明确标识上游/网关流中断的错误事件（`upstream_connection_error`、`upstream stream was interrupted`）属于确定性的上游故障，会在有限次重试轮后停止并明确报告故障，不再无限重试。已经开始输出工具调用的尝试，以及本身没有可用输出的失败（请求参数错误、上下文超限）仍立即丢弃重来。
...
- apply_patch 工具卡现在能在模型还在写参数时实时预览补丁：参数流式阶段卡片保持展开，显示不断增长的补丁文本并对 `+`/`-`/`@@` 做语法高亮，取代原来只显示字符数的提示；执行完成后无缝切换为最终 diff。
...
- 压缩后的 checkpoint 现在以 **会话锚点（session anchors）** 开头，并在递归压缩之间原样拷贝、不再重新推导：包括会话最初的请求、从你的纠正中提取的长期约束，以及你明确声明的约束（如「保持现有 API 行为不变」）。后来与旧约束矛盾的指令会取代旧约束，被取代的条目仍以 `~ ` 前缀留在锚点块里，模型能看到方向的改变，旧规则不会悄悄消失。checkpoint 还会把每个归档的 `history-N.md` 文件连同内容主题列成**历史地图**，需要原文时可以直接用 read 工具读回对应归档，不用猜该打开哪个文件。多次压缩后长会话的稳定性因此大幅提升。
