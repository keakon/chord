...
- TUI / Git 侧边栏：当当前目录位于 Git 仓库内时，右侧 info panel 现在会显示紧凑、可折叠的 Git 摘要，包括分支或 detached commit、linked worktree 名、改动文件数以及 ahead/behind 数量。Git 状态会在启动、文件修改工具或 Shell git 命令后异步刷新，并通过低频定时器更新，不阻塞渲染。
