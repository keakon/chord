...
- 后台 `spawn` 完成后现在必定在主会话留下结果卡片。此前主 agent 自己的后台任务卡片带着会被主视图过滤掉的归属，只能看到 toast 通知；空闲时完成的任务在后续回复之前完全不显示卡片；卡片事件在 UI 事件通道饱和时还可能被丢弃。所属 SubAgent 已不存在的结果现在显示在主会话中，而不是一个再也打不开的视图里。后台结果现在会先持久排队再展示：owner 正忙时，结果留在队列中，等下一个请求边界投递；即使在此期间 Chord 重启也不会丢。JOB RESULT 卡片只在结果消息真正持久追加进会话记录之后才建立，运行中的会话与恢复后的会话因此看到同样的卡片。完成的后台工作渲染为专用的 JOB RESULT 卡片，完成 toast 的级别也反映结果状态（取消、失败或成功），每个完成的后台对象各追加一条结果消息，而不会合并成一条。
