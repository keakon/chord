...
- 继续会话时会切回它当时所在的 checkout：`chord resume <id>` 与 `chord --resume <id>` 都会进入会话记录的 worktree；该 worktree 已不存在时先给出提示，再回退到主工作区继续。会话选择器用 `Worktree` 列标出它，不同 checkout 的会话一眼可辨。runtime cache 仍按 checkout 分开。
