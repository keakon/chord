...
- `delegate` 现在必须携带 `expected_write_scope`。只做研究时设为 `read_only: true`；否则至少声明 `files`、`path_prefix` 或 `modules` 其中一项。省略该字段或传入空范围都会被拒绝，不再静默取得全局独占范围；已有委派 prompt 和集成需要补上任务适用的最窄范围。
...
- `delegate` 现在可以携带受限任务能运行的命令。每次委派都必须声明 `expected_write_scope`，而任何非空写域都会拒绝全部 shell 调用——这意味着没有一个委派 worker 能构建、lint 或测试自己的产出：review worker 无法运行它被委派去跑的测试，验证委派工作完全落回 owner 身上，一次一个任务。写域声明现在接受 `verification_commands`：委派者为之背书的确切命令行。它们按字面匹配——不做前缀匹配、不允许额外参数——包含串联、替换或重定向的声明在委派时即被拒绝，因此一条看起来已获批准的条目无法变成任意执行。未授权任何命令的任务保持原有行为（完全没有 shell，验证是 owner 的事）；授权了命令的任务重新获得 `shell`，并在自己的 prompt 里被告知有哪些命令可用。
