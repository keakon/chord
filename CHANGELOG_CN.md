...
- shell 的窄 `allow` 规则现在对命令替换、进程替换和解析不了的引号也要求确认：命令里出现单引号外的 `$(...)`、反引号（双引号里的也算），或任何引号外的 `<(...)` / `>(...)`，以及引号没闭合或结尾是反斜杠时，不再自动命中 `"git *"` 这类具体 `allow` pattern，而是继续匹配后续规则；在同一条命令字符串里执行的函数体，以及决定命令名实际运行哪个二进制的前置 `PATH=` 赋值，也属于匹配内容，两者都不能借一个能命中的调用点藏下未经审查的命令。普通的重定向目标不在此列。
...
- 别名模型的 native thinking 目标现在解析正确：pin 为 `gemini` 时也会补上 Gemini 3 的 thought signature 占位值（别名的模型名看不出上游，缺签名的函数调用历史此前会直接 400），未设置 `native_thinking` 重新按 `auto` 处理：不再被当成 pin，上游家族仍按模型名推断，而不是退回 unknown。
