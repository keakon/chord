...
- `/yolo on|off`: temporarily bypass main-agent tool permissions while keeping handoff, delegate, cancel, and done permissions enforced. YOLO can be toggled while the agent is running; the execution-time permission bypass applies immediately to later tool calls, while the LLM-visible tool descriptions and permission prompt are refreshed on the next request. SubAgents inherit the mode while it is on: their ordinary `ask` tools stop prompting too, while `deny` rules and the protected control tools stay enforced. Toggling YOLO applies to a SubAgent's later calls immediately.
