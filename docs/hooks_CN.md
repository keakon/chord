...
`data` 内部具体字段会随版本演进。为了平稳：把没用到的字段当不透明对待，只依赖你真正需要的 key。
...
stdout 以纯字符串形式写入日志，没有 schema——想 print 啥就 print 啥，方便排错就行。
...
hooks:
  on_tool_call:
    - name: audit-bash
      command: ["./scripts/audit-bash.sh"]   # 或：shell: "./scripts/audit-bash.sh"
      timeout: 10                             # 秒，默认 30
      tools: ["Bash"]                         # glob 匹配 tool 名
      paths: ["src/**/*.go"]                  # glob 匹配相关路径
      agents: ["main", "reviewer"]            # glob 匹配 agent 名
      agent_kinds: ["primary", "subagent"]    # 精确匹配
...
所有 filter 是 AND 关系：每个非空 filter 都满足时才会跑。
