...
`edit`、`apply_patch` 和 `write` 的写入路径会清理模型泄漏进工具参数的零宽格式字符与孤立组合符号（零宽空格、零宽不连字、emoji 序列之外的零宽连字、词连接符、流中 BOM、软连字符；以及没有基字符可依附的变音符号，如游离的长音符）。这些字符不携带内容——没有基字符的变音符号无法改变任何字符的含义——清理不会改变文本含义；留着会把不可见字节写进文件。清理发生时，工具结果会报告具体清理了哪些码点（例如 `U+200B×2, U+FE0F×1, U+0304×1`），让模型学会不再输出它们。跟在字母后的组合符号（含堆叠变音）保持不变，越南语、阿拉伯语、天城文等合法文字不受影响。
...
孤立的变音符号（没有基字符可依附的 combining mark）在归一化时会被折叠掉：出现在行首、或紧跟空格、标点、数字之后的 mark，在真实文件内容里不会出现在那个位置，但复制标题（如 `### ̄.2.1`）被 token 化拆开后就会泄漏进来。跟在字母后的 mark 保持原样——它可能是阿拉伯语、天城文、越南语等文字的合法组合，包括一个字母携带多个变音符号的堆叠序列。
