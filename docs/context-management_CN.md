...
- **选哪一层**：[对比速览](#对比速览)。
- **持久摘要与 checkpoint**：[上下文压缩（Compaction）](#上下文压缩compaction)。
- **单次请求剪裁**：[上下文剪裁（Reduction）](#上下文剪裁reduction)。
...
主会话上下文使用量接近模型上限时，Chord 会准备自动压缩，并在准备下一次主模型请求时启动。压缩过程调用 LLM 分析当前对话，生成结构化摘要（目标、进度、关键决策、文件证据等），归档旧消息，用摘要替换对话历史。压缩结果持久保存到磁盘，会话文件体积显著缩小。
...
请求面的 reminder / warning overlay 只在启用 `model_driven` 时注入；关闭时自动压缩完全由运行时接管（与 Codex 的 local / remote 两条压缩路径一致，它们从不通知工作模型），会话只是继续跑，直到压缩在 barrier 应用。提醒只在会话继续发出主请求时生效：越线后若 turn 正好收尾，usage-driven 压缩走既有的 end-of-turn 路径。一次性外化提示只在真正启动压缩的那次请求上出现，宽限期内压缩还没启动、不会提前注入它。切换模型会套用新模型的 per-model 阈值并开启新的提醒窗口；若当前用量已经越过新模型的阈值，Chord 会把自动压缩置为待启动，由下一次过 gate 的请求与它并行执行——宽限期和压力提示与其他越线场景完全一致——而不是先把请求扣住：请求照常发出，只有 provider 因长度拒绝时才会先压缩再重试。
...
当前回合里最近几组失败的工具批次会作为真实记录重新挂在 checkpoint 卡片之后：被拒绝的调用（例如 runtime 驳回的 `compact_context` 请求）保留错误卡片，对该代做 fork 时也仍会回放，不会只剩卡片里的摘录。更早的失败只留在归档和 checkpoint 的证据包里。只有失败本身保留原文：同批里顺带成功的结果会换成 `[result elided by checkpoint: N bytes]` 标记，附件也不会被带回新上下文，完整输出在归档里。会话里这条标记会显示成一句「结果已被 checkpoint 归档」的说明；恢复后的 `edit` 卡片在应用 diff 被省略时回退显示请求的替换内容，改动过的文件也仍留在侧栏。
...
provider 同时公布「总上下文窗口」和「单独的输入上限」时，已知限制则建议三个字段都写明：
...
| 技能说明 | `skill` 加载的技能正文 | `stale_age_turns`（默认 3 轮后） | `stale_output_bytes`（默认 1500 字节以上才剪） | 与兜底规则共用同一组门（含 `min_tool_results_prune`），但按「指令」而非「数据」渲染：技能正文是模型正在执行的流程，且同名技能可能存在于多个加载根，只有信封里的 `<path>` / `<root>` 锚点能指认实际加载的那一份。摘要在保留这些锚点的同时给出正文的章节大纲，完整正文仍可从归档地址读回 |
...
| TODO 完成后的最终回复因 prompt cache 被扰动而更贵 | 保持 `wrap_up_grace_requests: 1`；只有你的流程通常在 TODO 完成后还会多一次验证请求，才考虑设为 `2` |
