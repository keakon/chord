...
| `delegate` | Start a delegated SubAgent workstream and return its startup handle (`task_id` / `agent_id`) immediately. It does not wait for completion. The call must include an `expected_write_scope`: set `read_only: true` when the task will not modify files, or declare the narrowest `files`, `path_prefix`, or `modules` scope that covers the work. The scope bounds the worker's file-modifying tools (targets outside the declared paths are rejected) and drives the parallel-scheduling guardrails; it does not remove command tools such as `shell`/`spawn`, which stay governed by the role's permission rules. An empty scope is rejected. Denying `delegate` also disables `cancel` and nested delegation for that role. |
