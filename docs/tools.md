...
| `delegate` | Start a delegated SubAgent workstream and return its startup handle (`task_id` / `agent_id`) immediately. It does not wait for completion. The call must include an `expected_write_scope`: declare the narrowest `files`, `path_prefix`, or `modules` scope covering the work. Whether a task may modify files is decided by its role's permission rules, not by the delegate call: a read-only task should pick an agent whose role denies the file-modifying tools (`write`, `edit`, `delete`, `apply_patch`) and pass an empty scope, which is accepted only for such roles. The declared paths bound the file-modifying tools the worker's role actually registers (targets outside the declared paths are rejected) and drive the parallel-scheduling guardrails; command tools such as `shell`/`spawn` are never scope-restricted and stay governed by the role's permission rules. An empty scope for a role that can write files is rejected. Denying `delegate` also disables `cancel` and nested delegation for that role. |
