...
如果主要现象是横线重复、输入区或状态栏分隔线重复、旧卡片边框残留，或右侧栏旧边框残留，最有用的问题是：这条多出来的线到底存在于 Chord 当前 frame，还是终端里残留的旧 frame。

建议按下面顺序取证：

1. 先截图，不要先调整窗口尺寸。截图应包含完整终端窗口，尤其是输入区、状态栏和右侧栏。
2. 立即导出 diagnostics bundle，尽量在 resize 之前完成。bundle 里会包含 `metadata.txt`、`tui-dump.txt` 和 `chord.log`。
3. 在 `tui-dump.txt` 中对照截图查看：
   - `[layout]` 与 `expected_separator_rows`：`main`、`info_panel`、`input`、`status`、`input_separator_row` 以及汇总的 `expected_separator_rows` 行列出 Chord 预期出现分隔线的所有行。
   - `[frame_bottom]` 与 `[screen_buffer_bottom]`：展示 Chord 最近渲染 frame 的底部行。
   - `[screen_buffer]`：展示 Chord 当前完整 screen buffer。
   - `[screen_buffer_horizontal_lines]`：列出 screen buffer 中所有包含水平制表线的行，便于把截图里多出来的线对应到具体行号，区分是 Chord 画的分隔线还是残留伪影。
   - `last_host_redraw`、`host_redraw_generation`、`host_full_frame`、`render_frame_generation`：判断当时是否走过 host redraw、fallback replay 或 Ghostty-safe full-frame 路径，以及截取的 buffer 来自哪一帧。
   - `recent_events`：关注截图时间附近是否有 `render-frame`、`host-redraw`、`host-redraw-skip`、`post-host-redraw-fallback-arm`、`image-protocol`、`focus`、`resize`、`scroll-flush` 或 `content-boundary` 事件。每当布局或分隔线行发生变化都会记录一条 `render-frame`，其 `separators` 与 `frame` 字段反映最近一次的分隔线几何。

判断方式：

- 如果多出来的线在 `[screen_buffer]` 中同一行也存在，说明 Chord 当前 frame 里很可能真的生成了重复分隔线。请保留 bundle，并记录对应行号。
- 如果截图里有这条线，但 `[screen_buffer]` 中没有，说明它更可能是旧 frame 在终端/renderer 中残留。这类情况通常与 host redraw、重新获焦、右边界 wrap 或 clear/repaint 时序有关。
- 如果伪影只出现在最右侧附近，或把终端宽度缩小一两列后就消失，优先怀疑右边界 wrap。请记录 `size` 以及 `host_full_frame safe_width/raw_width`。
- 如果伪影出现在图片预览、粘贴图片或导出 diagnostics 之后，也需要查看 `last_image_protocol_reason` 和 `last_image_protocol_summary`。

维护者进一步定位时，建议把最终 ANSI 输出拿到多个终端里 replay：如果同一份输出只在 Ghostty/libghostty 中留下旧线，而在 Terminal.app、iTerm2 或 WezTerm 中不复现，请同时保留 replay 文件和 Ghostty 截图；如果多个终端都显示同一条重复线，应优先排查 Chord 或 Bubble Tea/Ultraviolet renderer 的输出。

补充：从 v0.6.x 起，Chord 统一禁用 Bubble Tea/Ultraviolet 的硬件滚动优化（DECSTBM 区域滚动）。流式输出滚动时改为逐行就地重绘，规避部分终端（尤其是 libghostty 在窗口重新获焦的表面失效抖动窗口里）误处理 DECSTBM 区域滚动、把旧分隔线/边框留在屏幕上的问题。代价仅是每次滚动多发若干字节，本地 PTY 通常无感。`useFocusResizeFreeze` 全帧渲染、`safe_width` 右边界回避、以及 host-redraw / fallback 这一整套保护机制保留为正交防线。
