...
如果切换到大型 SubAgent transcript，或在 parked SubAgent 视图按 Enter 继续后整个 TUI 不再响应，当前版本会在焦点切换和继续路径中使用有界 transcript 窗口，并将上下文读取、rehydrate 与继续请求移出 TUI 更新热路径。若仍遇到无响应，先从另一个终端终止对应进程并用 `reset` 恢复原终端，然后导出 diagnostics。
