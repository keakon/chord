...
- YOLO removes the high-frequency confirmation friction of ordinary work — file edits and shell commands. It is **not** a redefinition of the role's boundary. Control tools change the agent topology or the session lifecycle rather than the risk of a single operation, and they are called rarely enough that confirming them was never the friction YOLO targets, so their permissions stay enforced: `handoff`, `delegate`, `cancel`, `done`, and `compact_context`. They split into two groups:
  - `handoff`, `delegate`, and `cancel` grant the role a capability it did not otherwise have — handing the session to another role, spawning delegated work, cancelling work it does not own. YOLO never authorizes these implicitly: under YOLO they are denied unless a rule names them directly, and a broad `"*": allow` is not enough. This is what keeps a single-agent role such as `builder` single-agent while YOLO is on. Turning YOLO on to skip edit confirmations is not a statement that the role should now be orchestrating SubAgents.
  - `done` and `compact_context` only end or shrink the current unit of work, so they follow the wildcard exemption above and stay usable. Denying them by default would break loop termination and silently disable model-driven compaction for anyone running YOLO.
  - SubAgents inherit the mode at execution time: while YOLO is on, their ordinary tools that would `ask` — file edits, shell commands — pass without raising the shared confirmation dialog, matching the main agent's own bypass. Inherited YOLO never relaxes `deny` (a read-only worker keeps its `write: deny`), and the control tools above stay enforced for them as well. The inheritance is live: switching YOLO off restores their confirmations on later calls.
