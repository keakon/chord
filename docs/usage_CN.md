...
MCP 工具调用会在卡片头部显示模型生成参数的紧凑摘要，JSON 值摘要为 `{N fields}` / `[N items]`，长值截断显示。MCP server 的启动参数与环境变量和工具调用参数彼此独立，不会被加入卡片。完成的工具卡片还会显示该次调用的墙钟耗时，结束的后台 `spawn` 任务会渲染为专用的 `JOB RESULT` 卡片，包含其命令、状态与输出。
