...
工具卡片会用终端安全的方式展示预览。会话工作目录内的文件显示相对路径，外部文件显示绝对路径。工具路径参数支持 `~/...` 前缀，会展开到用户主目录。`read`、`grep`、`glob` 和成功的 `shell` 输出默认折叠；折叠态只保留行范围、匹配数量、命令意图和终态等关键信息，完整输出、诊断、截断详情和 artifact 引用在展开后查看。`write`、`edit` 卡片默认折叠，执行完成后展开显示完整 diff。`apply_patch` 卡片则默认展开：参数还在流式到达时，不断增长的补丁文本（`+`/`-`/`@@` 高亮）就是卡片的主要内容；每个 `@@` hunk 会在同一行带出分隔线，但代码仍按行高亮。补丁失败且没有实际应用的 diff 时，卡片会保留这份请求补丁预览，并与错误信息分开标记；成功执行后才切换为最终 diff。终态标记旁的 `▸` / `▾` 表示卡片可以展开或收起，聚焦后按 `Space`、`Enter` 或 `o` 即可切换。`delete` 不允许折叠：删除理由，以及已删除、不存在、被阻止、失败和未尝试的全部路径都会直接显示。无论卡片当前是否展开，`yy` 都会复制底层完整内容。

MCP 工具调用会在卡片头部显示模型生成参数的紧凑摘要，JSON 值摘要为 `{N fields}` / `[N items]`，长值截断显示。MCP server 的启动参数与环境变量和工具调用参数彼此独立，不会被加入卡片。已完成工具卡片会把调用的墙钟耗时附在卡片头部，按整秒显示，不足 1 秒时隐藏；`question` 卡不显示耗时标签——它计时的是用户作答的时间，不是 agent 的工作——思考卡和助手正文卡同样不显示单卡耗时。结束的后台 `spawn` 任务会渲染为专用的 `JOB RESULT` 卡片，包含其命令、状态与输出。
