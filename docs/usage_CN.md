...
工具卡片会用终端安全的方式展示预览。会话工作目录内的文件显示相对路径，外部文件显示绝对路径。`read`、`grep`、`glob` 和成功的 `shell` 输出默认折叠；折叠态只保留行范围、匹配数量、命令意图和终态等关键信息，完整输出、诊断、截断详情和 artifact 引用在展开后查看。`write`、`edit`、`apply_patch` 默认完整展示改动。`apply_patch` 参数还在流式到达时，卡片会保持展开并显示不断增长的补丁文本（`+`/`-`/`@@` 高亮），执行完成后切换为最终 diff。终态标记旁的 `▸` / `▾` 表示卡片可以展开或收起，聚焦后按 `Space`、`Enter` 或 `o` 即可切换。`delete` 不允许折叠：删除理由，以及已删除、不存在、被阻止、失败和未尝试的全部路径都会直接显示。无论卡片当前是否展开，`yy` 都会复制底层完整内容。
