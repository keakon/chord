...
权限确认弹窗也可以用 `M` 添加记住规则。在规则选择器中按 `E` 可在保存前编辑建议 pattern。Delete 确认优先列出保守的路径级建议（精确路径和同目录 pattern），且始终提供全局 `*` 通配项；文件位于当前工作目录内时，也会提供一个 `**` 递归规则选项（相对当前目录），选了之后在嵌套子目录里的删除可以直接放行，不用反复确认。
