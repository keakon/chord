...
`write`、`edit`、`apply_patch`、`todo_write`、`delete`、`handoff` 这些卡片恒展开，正文（文件内容、diff、todo 列表、错误详情或计划路径）直接可见，标题不带折叠标记。`read`、`grep`、`glob`、成功的 `shell` 输出和通用工具调用默认收起、仍可折叠。`apply_patch` 卡片在参数流式到达时展示不断增长的补丁文本（`+`/`-`/`@@` 高亮），每个 `@@` hunk 在同一行带出分隔线，代码仍按行高亮。补丁失败且没有实际应用的 diff 时，卡片保留这份请求补丁预览，并与错误信息分开标记；成功执行后才切换为最终 diff。无论卡片当前是否展开，`yy` 都会复制底层完整内容。

MCP 工具调用会在卡片头部显示模型生成参数的紧凑摘要，JSON 值摘要为 `{N fields}` / `[N items]`，长值截断显示。MCP server 的启动参数与环境变量和工具调用参数彼此独立，不会被加入卡片。已完成工具卡片会把调用的墙钟耗时附在卡片头部，按整秒显示，不足 1 秒时隐藏；`question` 卡不显示耗时标签——它计时的是用户作答的时间，不是 agent 的工作——思考卡和助手正文卡同样不显示单卡耗时。结束的后台 `shell` 任务会渲染为专用的 `JOB RESULT` 卡片：默认只显示每个 job 的标题行与一行状态，展开后可见完整的命令、状态与输出。
