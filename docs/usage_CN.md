...
MCP 工具调用会在卡片头部显示模型生成参数的紧凑摘要，JSON 值摘要为 `{N fields}` / `[N items]`，长值截断显示。MCP server 的启动参数与环境变量和工具调用参数彼此独立，不会被加入卡片。已完成工具卡片会把调用的墙钟耗时附在卡片头部，按整秒显示，不足 1 秒时隐藏；思考卡和助手正文卡不显示单卡耗时。结束的后台 `spawn` 任务会渲染为专用的 `JOB RESULT` 卡片，包含其命令、状态与输出。
