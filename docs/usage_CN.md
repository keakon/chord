...
工具卡片会用终端安全的方式展示预览。会话工作目录内的文件显示相对路径，外部文件显示绝对路径。工具路径参数支持 `~/...` 前缀，会展开到用户主目录。能折叠的卡片默认收起，只保留行范围、匹配数量、命令意图和终态等关键信息；完整输出、诊断、截断详情和 artifact 引用展开后才显示。终态标记旁的 `▸` / `▾` 表示这张卡可以展开或收起，聚焦后按 `Space`、`Enter` 或 `o` 切换。

`write`、`edit`、`apply_patch`、`todo_write`、`delete`、`handoff` 这些卡片恒展开，正文（文件内容、diff、todo 列表、错误详情或计划路径）直接可见，标题不带折叠标记。`read`、`grep`、`glob`、成功的 `shell` 输出和通用工具调用默认收起、仍可折叠。折叠后已经完整显示正文的卡片——例如只占一行的失败原因——同样不带标记、不能折叠。后台 `shell` 卡片折叠时把 job 句柄标在标题行（`shell … · job-8`），`job_list` 折叠时显示任务数量（`job_list · 3 jobs`），不必为了确认目标而展开。`apply_patch` 卡片在参数流式到达时展示不断增长的补丁文本（`+`/`-`/`@@` 高亮），每个 `@@` hunk 在同一行带出分隔线，代码仍按行高亮。补丁失败且没有实际应用的 diff 时，卡片保留这份请求补丁预览，并与错误信息分开标记；成功执行后才切换为最终 diff。无论卡片当前是否展开，`yy` 都会复制底层完整内容。

MCP 工具调用会在卡片头部显示模型生成参数的紧凑摘要，JSON 值摘要为 `{N fields}` / `[N items]`，长值截断显示。MCP server 的启动参数与环境变量和工具调用参数彼此独立，不会被加入卡片。已完成工具卡片会把调用的墙钟耗时附在卡片头部，按整秒显示，不足 1 秒时隐藏；`question` 卡不显示耗时标签——它计时的是用户作答的时间，不是 agent 的工作——思考卡和助手正文卡同样不显示单卡耗时。结束的后台 `shell` 任务会渲染为专用的 `JOB RESULT` 卡片：默认每个 job 只显示一行标题，只有失败、取消或耗时等标题行表达不了的状态才会再占一行；展开后可见完整的命令、状态与输出。

右侧信息面板的 `TIME` 区块把聚焦 agent 的已用墙钟时间拆成 `Model`（模型）、`Tools`（工具执行）、`Cooldown`（冷却等待）、`User wait`（用户等待）四类，并显示各自占比。不足 1 秒的类别不显示——否则会展示成 `0s (1%)`——任一类别达到 1 秒后该区块才会出现。上下文压缩耗时直接计入 `Model`，不单独显示。
...
恢复会话时，Chord 只会重建允许后续编辑所需的安全状态，不会重新 Read 文件，也不会把文件内容重新载入对话。如果某个文件自会话结束以来在磁盘上已变化，或它的读取历史没有持久化记录，那么下次对它的 `edit` / `apply_patch` 可能被拦截，agent 会先重新 `read` 再编辑。这是有意为之：避免基于过期文件内容做编辑。
...
Memory 是 Chord 的可选跨会话记忆：把用户偏好、稳定的项目事实和可复用工作流沉淀为项目内的可检索记录，减少重复说明。它默认**不自动启用**——需要 `memory.enabled: true` 显式开启自动抽取——读取与编辑始终可见。
...
- 当这份 Memory 摘要已经注入到当前会话时，对这一轮来说它就等同于当前 `MEMORY.md` 的已读副本。Chord 不应为了重新打开 `MEMORY.md` 本身、或“确认”和注入内容一致而额外做一次文件读取。真正需要核验的是索引中的结论：按需打开对应 record，或更优先用代码、测试、文档等一手证据确认。
...
- 自动抽取在会话冻结后后台进行，只在主 agent 空闲时调用模型，并使用主模型池的完整 fallback 语义；新用户消息到来时会被抢占取消，稍后重试。抽取模型会同时看到有界的 `AGENTS.md` 规则和当前活跃记忆，用来排除已经由项目规则承载的内容、跳过重复结论，并用新结论替代已过时的索引项。当前分支、提交、push、rebase、worktree 等临时状态不会成为长期记忆。
- 每条自动记录都有类型（偏好/事实/工作流/教训）、来源和认知状态；assistant 的历史陈述不会被升级为已验证事实。正文会分别说明结论、为什么值得跨会话保留，以及以后在什么场景下怎么用；没有实质展开价值的短句不会为了凑索引而单独建 record。完全重复的结论不会再次写入；修订后的记录会替代 `MEMORY.md` 中的旧索引项，旧 record 仍作为来源记录保留。
- **索引会被整理，不只是追加。** 抽取同时会判断已有条目：不再成立、本来就不该记、或已经由项目规则表达的，会被移出索引，record 文件仍留作来源。你亲口说过的内容不在此列——只有 assistant 自己总结的条目才能这样移出。索引涨到注入预算能装下的条数后，会额外跑一轮只看索引本身（不带会话记录）的整理，把规模压回预算之内，早期或较弱模型写下的记忆因此不会变成永久负担。
...
- 抽取前后都会做敏感信息清洗（token/key、URL 凭据、PEM/私钥块、高危环境变量赋值等），但这只是 best-effort 防护，不承诺识别任意自定义秘密格式——普通的 Git diff 审阅仍是必要的边界。`MEMORY.md` 中 `<!-- chord:managed:start -->` 与 `<!-- chord:managed:end -->` 之间的内容是 Chord 自动维护的索引，之外的内容由你维护，Chord 不会覆盖。删除记录时直接删掉索引里对应的行即可——record 文件会保留为 orphan，不会被自动挂回，你也可以自行删除该文件。
- 同一项目还有别的 Chord 会话在跑时，别手改 `MEMORY.md`：后台提交会同时重写 managed 区，可能撞车。先关掉那些会话，或者交给自动整理去做。
- 自动抽取开启时，状态栏会显示 `MEMORY` 标识（与 `LOOP` / `YOLO` 一致）。
...
Chord 会根据当前模型选择文件编辑工具：gpt-5 及之后主版本家族（`gpt-5`、`gpt-5-mini`、`gpt-5-nano`、`gpt-5-codex`、任意 `gpt-5.*` 名称，以及未来的 `gpt-6` 等）和 `codex-auto-review` 使用 `apply_patch`，其余模型默认 `edit`——完整矩阵和依据见 [编辑工具](./edit-tools_CN.md)。在兼容的 Responses 端点上，补丁原生模型还会把 `apply_patch` 以 freeform custom tool 形态发射，而不是 JSON function tool。

当模型名或网关的实际表现与推断不符时，可以用 `compat.apply_patch.enabled`（工具面）和 `compat.apply_patch.freeform`（发射形态）按 provider 或模型覆盖。两个键都是三态：省略表示按模型名和端点推断，只需设置要改的那个。字段权威说明见 [配置与认证](./configuration_CN.md)。
