...
这些数据是特定场景下的依据，不是对所有任务的通用承诺。它们主要展示 Chord 的上下文剪裁、低开销 TUI 和内存占用在哪些地方有价值。硬件、运行环境、会话内容、模型行为和实现路径都会影响结果。

下面两张表都标注了各自的测试版本。

### 真实编码任务

我们让六款 agent harness 跑了同一个 [DeepSWE v1.1 任务](https://deepswe.datacurve.ai/data/v1.1/tasks/httpx-streaming-json-iteration)：给 `httpx` 加上流式 JSON 迭代接口。Chord 用时最短、成本最低：6m37s、￥0.348，第二名的耗时和成本都是它的 1.5 倍。
...
|---------|------|----------|-------------|-------------|-----------------|------|
| **Chord v0.8.1** | **6m37s** | **49 次** | **54,530** | **58,559** | **2,961,280** | **￥0.348** |
| deepseek-harness 0.1.5-rc.1 | 9m50s（1.5 倍） | 93 次 | 58,569 | 80,314 | 7,357,440 | ￥0.527（1.5 倍） |
| pi 0.85.1 | 10m22s（1.6 倍） | 101 次 | 79,189 | 94,733 | 5,871,488 | ￥0.576（1.7 倍） |
| codex 0.154.0 | 17m01s（2.6 倍） | 121 次 | 72,363 | 135,310 | 15,787,264 | ￥0.929（2.7 倍） |
| mini-swe-agent 2.4.6 | 18m29s（2.8 倍） | 158 次 | 161,893 | 77,320 | 18,206,592 | ￥0.835（2.4 倍） |
| claude code 2.1.272 | 22m25s（3.4 倍） | 143 次 | 106,546 | 214,126 | 7,024,768 | ￥1.104（3.2 倍） |
...
- 成本按该模型的标价估算，每 100 万 token：输入 ￥1、输出 ￥4、缓存读取 ￥0.02。
