...
Chord 自主连续执行任务的模式，适合长时间任务。loop 模式下 agent 会持续处理任务、工具和结果，直到通过 `done` 工具申请退出。为保留工作状态，loop 模式会对新增消息禁用请求级上下文剪裁；上下文压缩仍保持启用，以便长会话在上下文额度用尽后继续推进。
...
Chord 在模型响应仍在流式传输时（工具参数刚完整），就提前执行一小批安全工具，缩短"最终确认等待"时间。可提前执行的是只读工具以及 `todo_write` 预览；文件变更工具不会推测执行。始终开启，不可配置。详见 [使用指南 — 工具执行细节](./usage_CN.md#工具执行细节)。
