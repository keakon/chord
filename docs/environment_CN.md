...
| `CMUX_SOCKET` / `CMUX_SOCKET_PATH` | 检测 Chord 跑在 cmux 内；影响图片协议管线与 TUI 刷新节奏                                  |
| `NO_COLOR`              | 任意非空值会禁用启动时 stderr 的 ANSI 颜色                                                    |
| `USER` / `USERNAME`     | 部分诊断输出会用到                                                                              |

Chord 会对所有终端禁用终端硬滚动优化，因为这些滚动序列可能在 Chord 的 sticky transcript 布局中留下残影。检测到 cmux 时，Chord 还会使用更保守的 TUI 策略，因为 cmux/libghostty 嵌入层处理高频小块终端更新时，CPU 开销可能明显高于 Ghostty.app。该策略是自动的，没有用户配置项：cmux 会降低流式输出/滚动刷新频率，并关闭仅前台可见的视觉动画 tick，同时保留 terminal title 动画，方便从后台标签观察运行状态。
...
环境里没有 `GOGC` 时，Chord 会把 GC 目标设为 `GOGC=50`，TUI 与 `chord headless` 统一适用。长会话会一直握着一大批消息，目标压低后常驻堆更平稳，不会一路涨向默认目标，代价是多花一点 CPU。显式设置的 `GOGC` 始终优先。

`GOMEMLIMIT` 交给 Go runtime 直接读取，Chord 不做改动。只有在 Chord 跑在外部内存预算下时才需要设它：

- **TUI**：不要设 `GOMEMLIMIT`。桌面会话没有外部预算，而上限压到接近实际存活堆会让 GC 一直运行，表现就是滚屏和流式输出变慢。
- **容器里的 `chord headless`**：把 `GOMEMLIMIT` 设在容器上限略低处（常见取法是上限的 90% 左右），给进程留出余量、别被 OOM 杀掉；也别低于进程常规占用，太低会开始 GC 空转。默认的 `GOGC=50` 保留即可，两个变量同时生效。
...
`CHORD_API_BASE` 会覆盖本次运行所有 provider 的基础 URL，效果等同于传 `--api-base`；两者同时设置时 CLI flag 优先。由于它作用于该次运行的全部 provider，长期的按 provider 覆盖建议直接在 `config.yaml` 里给各 provider 配 `api_url`。
