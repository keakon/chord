...
- `question_follow_up_at_end`: when `true` on a MainAgent role, Chord injects a completion follow-up prompt requirement only when that feature is enabled and the `Question` tool is available. Purpose: keep the post-completion handoff structured so the user can continue directly from the follow-up prompt instead of relying on a plain-text stop. Typical scenarios: long-lived coding sessions where one task often rolls directly into the next, and loop-driven runs that should finish the current task but remain ready for the next instruction. In normal non-loop turns, the model is guided to finish the current task, write a concise completion summary, and then call `Question` as the final action in the turn so the user can continue with another task or explicitly end the session. In loop mode, completion still requires the assistant response to include `<done>reason</done>`, and when this follow-up feature is enabled the completion response should also hand off through `Question` instead of stopping with summary text alone. When the feature is disabled, or `Question` is unavailable, Chord does not add this extra prompt guidance and existing completion behavior is unchanged. If `Question` is not explicitly configured in `permission`, Chord auto-adds `Question: allow` at the role base layer so the tool is visible to the model. Explicit `Question: deny` disables this feature entirely (no follow-up prompt injection). `Question: ask` is treated as `allow` to avoid a redundant second confirmation step before the user can answer the question.
