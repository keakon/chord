...
| `compaction`      | object | Per-model compaction overrides: `compaction.threshold` (auto-compaction usage ratio; `0` disables for this model) and `compaction.reminder` (pressure-reminder line; `0`/absent derives `min(0.60, threshold×0.90)`). Unset fields inherit the global `context.compaction.*`. Out-of-range values are rejected with a warning and inherit the global value. See [Context compaction](./context-management.md#context-compaction). |
