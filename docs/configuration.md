...
| `context.compaction.model_driven` | bool | Experimental opt-in: expose the `compact_context` tool to the main agent so the model can request a durable context checkpoint once its working state is externalized. See [Model-driven context checkpoint](./context-management.md#model-driven-context-checkpoint-experimental). Default `false`; a project-level `false` overrides a global `true`. |
