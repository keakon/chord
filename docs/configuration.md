...
- `question_follow_up_at_end`: when `true` on a MainAgent role, Chord injects a completion follow-up prompt requirement that tells the model to do one of two things before ending a completed turn: either confirm the user already explicitly said they are done / want to end / do not want further help / have no more tasks, or call `Question` in the current turn. The prompt also tells the model not to treat a written completion summary as the end of the turn, to verify completion / verification status / todo sync first, and then make the `Question` call as the final completion step. It also explicitly requires a real `Question` tool call rather than printing tool-like JSON or pseudo-tool syntax in assistant text. This is prompt-level guidance for the model, not a hard runtime guarantee that the model will always comply. If `Question` is not explicitly configured in `permission`, Chord auto-adds `Question: allow` at the role base layer so the tool is visible to the model. Explicit `Question: deny` disables this feature entirely (no follow-up prompt injection). `Question: ask` is treated as `allow` to avoid a redundant second confirmation step before the user can answer the question.
