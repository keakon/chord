...
Use `root_markers` when a language server should run across specific project directories. If omitted, `file_types` alone controls whether the server handles a file.
...
For Python, `root_markers` is usually better left unset. Turning them on scopes Pyright to marker directories, which can make valid standalone scripts or lightweight projects fail to enable Pyright. If you need stricter project scoping, add `root_markers` explicitly and the workspace root will re-root to the nearest `pyproject.toml`/`pyrightconfig.json` directory as above.
