...
本页详细说明 Chord 读写的所有文件和目录，以及如何安全地清理它们。

## 三层布局

| 层级               | 默认路径                                                | 用途                                                                                               |
| ------------------ | ------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| **配置主目录**     | `$XDG_CONFIG_HOME/chord` 或 `~/.config/chord`           | 你编辑的用户配置：providers、模型池、自定义 agent、自定义 skill、自定义 slash 命令                  |
...
└── logs/
    ...
    ├── chord.log.1                     # 轮转
    ├── chord.log.2                     # 轮转
    └── tui-dumps/                      # `Ctrl+G` / `/diagnostics` 输出
```

### `<project-key>` 是什么？

Chord 用项目的 canonical 文件系统根（解析符号链接、规范化大小写）作为身份，再据此推导一个稳定、清洗后的 key——例如 `~/projects/chord` 的 key 是 `HOME-projects-chord`。如果两个项目清洗后冲突，Chord 会追加 8 字符指纹消歧。完整的 canonical root 也会写入 `project.json`，所以即使路径长得像，注册表也不会混淆。

Sessions、运行时缓存、exports 都以这个 key 为索引——所以你在 `~/projects/chord` 重新跑 `chord` 时能找到上次的会话。

### Worktree

`chord --worktree <name>` 会在 `worktrees/<repo-id>/<slug>` 下创建 chord 管理的 git worktree，**位于原仓库之外**，并拥有自己的 project key。每个 chord-managed worktree 的 sessions、cache、exports 因此天然隔离。

要清理 worktree（仅删 chord 一侧的数据），用 `chord worktree remove <name>`——见 [CLI — chord worktree](./cli_CN.md#chord-worktree)。**不要**手动删 worktree 目录，那会留下注册表中的孤儿条目（之后会被 `chord cleanup project` 标记）。

## cache 目录 — `~/.cache/chord/`

这里都是可重建数据，任何时候都可以删，代价仅是一次重新预热。
...
`chord` 首次在某项目启动时会按需创建项目根的 `.chord/`。这是**唯一**位于用户仓库内部的 chord 目录。
...
| `<state-dir>/logs/tui-dumps/`          | `Ctrl+G` / `/diagnostics` 生成的诊断快照（用于报 bug）                |
...
把 key-value 片段当人类可读文本看，不是稳定的结构化日志 schema。

## 维护

请优先使用 `chord cleanup`，**不要**直接 `rm -rf`——前者了解哪些路径删了会留下孤儿注册项。
...
| `<state-dir>/exports/<project-key>/`              | 可以——这些是 `/export` 的输出，用户向。                                                                              |
| `<state-dir>/sessions/<project-key>/<sid>/`       | 如果你确定要丢这个 session 的历史，可以。更建议 `chord cleanup sessions --older-than …`。                           |
| `<state-dir>/sessions/<project-key>/`             | **不建议**：会丢这个项目**所有**会话。                                                                               |
| `<state-dir>/projects/<project-key>.json`         | **不建议**：手改会让注册表不一致。请用 `chord cleanup project`。                                                     |
| `<state-dir>/worktrees/...`                       | **不建议**：用 `chord worktree remove <name>`。                                                                      |
| `~/.config/chord/`                                | 仅当你想做完全重装时。删 `auth.yaml` 之前请确保 key 还在别处。                                                       |
| `<project>/.chord/`                               | 仅当你确实想丢弃项目级 chord 配置时。这个目录通常是入 git 的。                                                       |
