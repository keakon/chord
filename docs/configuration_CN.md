...
对 `type: responses`，Chord 对所有 provider 使用同一套稳定的 Responses 请求形态：稳定 system prompt 放在顶层 `instructions` 字段；对话消息仍保留为 typed `input` item，例如 `{"type":"message","role":"user",...}`。请求还会显式发送 `tool_choice`、`parallel_tool_calls`、`store`、`stream` 和 `include` 数组。Responses 与 Chat Completions provider 的 `parallel_tool_calls` 都默认是 `true`，允许模型在一次响应中返回相互独立的工具调用；如果后端或工作流要求串行调用，可在 model 上设置 `parallel_tool_calls: false`，也可在 model variant 中覆盖。除 `preset: azure` 外，`store` 默认是 `false`，显式 provider / model 配置会覆盖这个默认值。没有 reasoning 块时 `include` 为空，有 reasoning 块时才包含 `reasoning.encrypted_content`。Responses 请求不会序列化 `max_output_tokens`。流式 Responses 模型请求会带上 `Accept: text/event-stream` 和 `User-Agent`。非 Azure 的 Responses 请求还会带上 Codex 兼容的 `OpenAI-Beta: responses=experimental` 和 `originator` header，因为有些中转站会先校验这套形态再转发请求。默认 User-Agent 仍是 `chord/<version>`，除非 provider 级 `user_agent` 显式覆盖。
...
model_templates:
  ...
  # Claude 自适应思考变体（只需在变体中设置 effort，type 和 display 继承自基础配置）
  ...
  # Claude 自适应思考模板（4.6+ 所有版本）
  claude-adaptive: &claude-adaptive
    <<: *claude-base
    thinking:
      type: adaptive
      display: summarized  # 通用配置：所有变体都会继承
...
- `reasoning`：OpenAI / OpenAI-compatible reasoning 选项。
  - `reasoning.effort` 用来控制 Chord 主动请求的 reasoning 深度/预算。Chord 会先做大小写/空格归一化，再原样透传该字段，因此 provider 支持的任意取值（包括 GLM 的 `max` / `minimal` / `none`）都能不变地到达上游；留空表示不发送该字段，使用 provider/model 自身默认值。仅官方 Codex（`preset: codex`）Responses 后端会把不属于 `low` / `medium` / `high` / `xhigh` 的取值以 warn 丢弃。日常编码一般可从 `medium` 起步；`high` / `xhigh` 更适合复杂规划、疑难调试或高难度综合任务，但通常也会增加延迟和 token 消耗。
...
- `compat.reasoning_continuity`：可选的、面向特定协议的 reasoning/thinking 连续性兼容开关，用于那些兼容 API 明确要求客户端回放额外思考载荷的模型。
  - 默认行为偏保守，普通 GPT / Claude / Gemini 配置**不需要**额外设置：
    - `type: responses`：Chord 会在 transport 支持时使用 Responses API 自带的连续性机制（例如支持的 transport 上使用 `previous_response_id`），**不会**把可见 reasoning 文本回放进请求历史。
    - `type: messages`：当 Anthropic thinking 开启时，Chord 会按 Messages 协议回放需要的 thinking/signature blocks。
    - `type: chat-completions`：Chord 会保留普通可见 assistant 文本和 tool history，但**不会**回放 assistant `reasoning_content`；只有当某个 model/provider 的文档明确要求时，才显式启用 `compat.reasoning_continuity.mode: openai_visible`。
    - `type: generate-content`：Chord 使用 Gemini 的常规请求格式，不额外增加 Chord 自定义的可见 thinking 回放层。
  - 只有当某个 OpenAI-compatible `chat/completions` 模型的官方文档明确要求回放可见 `reasoning_content` 时，才使用 `compat.reasoning_continuity.mode: openai_visible`，例如 GLM 的 Preserved Thinking。
  - `openai_visible` 仅对 `type: chat-completions` 生效。启用后，Chord 会在历史消息里保留 assistant 的 `reasoning_content`，并在需要时自动发送对应 provider 的 preserved-thinking 请求标记。
  - 如果 provider 级别设置了 `openai_visible` 默认值，但某个具体模型不适用，可在该模型上设置 `mode: none` 覆盖并关闭它。
  - 跨类型切换默认采取保守策略：如果目标协议无法表达源模型的 continuity payload（例如 `responses -> messages` 或 `messages -> responses`），Chord 会保留普通可见对话和 tool history，但自动丢弃不兼容的 reasoning/thinking continuity 载荷，不做不安全的协议间转换。

  对 GLM-5.2，要把 OpenAI-compatible 和 Anthropic-compatible 模板分开。`type: chat-completions` 使用 OpenAI 模板；只有当网关明确提供 Responses endpoint 时，`type: responses` 才使用 OpenAI 模板；`type: messages` 使用 Anthropic 模板。Chord 会把 `reasoning.effort` 映射到 OpenAI-compatible 请求字段，把 `thinking.effort` 映射到 Messages 的 `output_config.effort`。1M 上下文在 Chord 中通过 `limit.context: 1000000` 表达，模型名仍写 `glm-5.2`。
...
- `modalities.input`：模型支持的输入类型，可选 `text`、`image`、`pdf`。省略时默认仅 `[text]`；只有确实支持图像 / PDF 输入的模型才应显式声明 `image` / `pdf`。
- 如果请求里包含目标模型不支持的 `image` / `pdf` part，Chord 会在发送前直接丢弃这些 part，而不是继续把不支持的二进制输入转发给上游。依赖图像回放的工具（如 `view_image`）也只会在当前模型 / tool surface 支持时才对模型可见。
...
- `response_header_timeout`：该 provider 流式模型请求的初始 HTTP 响应头超时。它用于限制连接成功后迟迟没有首个响应的等待时间，不限制健康流的总耗时；流式 chunk 之间的最大空闲时间由 `stream_idle_timeout` 控制。`0` 保持内置响应头超时默认值。
...
| `response_header_timeout` | int | 该 provider 流式模型请求的初始 HTTP 响应头超时，单位秒。`0` / 省略表示使用内置响应头超时默认值；健康流由 `stream_idle_timeout` 约束，而不是总请求计时器。 |
...
| `reasoning`       | object | OpenAI reasoning 选项。`reasoning.effort` 会先归一化再原样透传，因此 provider 支持的任意取值（如 GLM 的 `max` / `minimal` / `none`）都能不变地到达上游；仅官方 Codex Responses 后端额外限制为 `low` / `medium` / `high` / `xhigh`（留空 = 不发送，使用 provider/model 默认）。Responses 场景下还支持 `reasoning.summary`（`auto` / `concise` / `detailed`；留空 = 不发送 / 不显式请求 summary）。需要可读摘要时推荐 `auto`。 |
...
| `compat.reasoning_continuity.mode` | string | 可选的协议兼容覆盖项，用于控制特定模型的 reasoning/thinking 连续性。普通 GPT / Claude / Gemini 配置建议留空；只有当某个 OpenAI-compatible `chat/completions` 模型官方明确要求回放可见 `reasoning_content`（例如 GLM Preserved Thinking）时，才设置为 `openai_visible`。模型级 `none` 可覆盖并关闭 provider 级默认值。 |
