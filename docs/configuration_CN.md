...
model_templates:
  gpt-400k: &gpt-400k
    ...
    modalities:
      input: [text, image]
  # anchor/alias 名不要包含点号；下面统一使用连字符。
  gpt-5.2: &gpt-5-2
    <<: *gpt-400k
    cost:
      input: 1.75
...
- `reasoning`：OpenAI / OpenAI-compatible reasoning 选项。
  - `reasoning.effort` 用来控制 Chord 主动请求的 reasoning 深度/预算。Chord 会先做大小写/空格归一化，再原样透传该字段，因此 provider 支持的任意取值（包括 GLM 的 `max` / `minimal` / `none`）都能不变地到达上游；留空表示不发送该字段，使用 provider/model 自身默认值。日常编码一般可从 `medium` 起步；`high` / `xhigh` 更适合复杂规划、疑难调试或高难度综合任务，但通常也会增加延迟和 token 消耗。
