...
Gemini 的 `api_url` 应设为 `/models` 基础路径。Chord 根据 `/models` 后缀自动识别为 `type: generate-content`，可省略 `type`。不要在 URL 中包含模型名或 `:streamGenerateContent?alt=sse`，Chord 会自动追加 `/{model}:streamGenerateContent?alt=sse`。`models` 下的 key（如 `gemini-3.5-flash`）即为发送给 Gemini 的模型 ID。
...
- `request_timeout`：该 provider 的 HTTP 请求总超时。适用于 HTTP/SSE 模型请求；`0` 表示不设置总请求超时。
...
这些配置按 provider 生效，因此项目级 `.chord/config.yaml` 可以只覆盖某一个 provider 的超时，不影响其他 provider。它们不会改变底层固定连接默认值，例如 TCP dial、TLS handshake 或 HTTP response header timeout。
...
自动压缩由 provider 返回的输入 usage 触发。请求级剪裁可能让当前 prompt 变小，但剪裁后得到的本地估算不会取消已经由 provider usage 触发的压缩请求。
...
provider usage 是自动触发的权威依据。Chord 不会用请求级剪裁后的本地 token 估算去清除已经触发的自动压缩请求，因为多模态输入、工具 schema、provider/proxy framing 等都可能让本地估算与 provider 统计不一致。
...
| `request_timeout` | int | 该 provider 的 HTTP 请求总超时，单位秒。`0` / 省略表示不设置总请求超时。 |
