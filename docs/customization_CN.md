...
会话开始时，Chord 会从当前工作目录向上探索到项目根目录，发现适用的 `AGENTS.md` 文件；随后按“项目根目录 → 当前工作目录”的顺序注入每个非空文件的完整内容，并用相对当前工作目录的路径作为段落标题（例如从嵌套目录运行时会显示 `## ../../AGENTS.md`、`## ../AGENTS.md` 和 `## AGENTS.md`）。如果当前工作目录就是项目根目录，则只加载根目录的 `AGENTS.md`。这些指令会作为内部 `<system-reminder>` user-role 消息注入 LLM 请求，位置在第一条真实用户消息之前。这个 meta message 可能不会显示在可见对话记录里，但 main agent 与 sub-agent 都会把它当作持久工作区指导来遵守，除非它与更高优先级的 system、developer 或 user 指令冲突。

Chord 也会为当前会话探测一个 Python 虚拟环境：从当前工作目录向上探索到项目根目录，并在每一层按 `.venv`、`venv`、`env` 的顺序检查。找到第一个有效环境后，prompt 会用相对当前工作目录的路径提示该环境，并要求 agent 运行 Python 命令时优先使用其中的解释器。
