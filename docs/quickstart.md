...
Based on that endpoint, Chord recommends a starter provider name and model such as `openai` / `gpt-6-astra`, `anthropic` / `claude-opus-5`, or `gemini` / `gemini-3.5-flash`.
