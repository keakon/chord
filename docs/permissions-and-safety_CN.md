...
- YOLO 消除的是日常工作中高频确认带来的摩擦——文件编辑和 shell 命令，它**不是**对角色边界的重新定义。控制工具改变的是 agent 拓扑和会话生命周期，而不是单次操作的风险面；它们调用频率很低，确认它们本来就不是 YOLO 想消除的那种摩擦，因此这些工具的权限仍然生效：`handoff`、`delegate`、`cancel`、`done`、`compact_context`。它们分为两类：
  ...
  - SubAgent 在执行时继承该模式：YOLO 开启期间，它们原本需要 `ask` 确认的普通工具（文件编辑、shell 命令）会直接放行，不再弹出共享确认框，与主 agent 自身的绕过行为一致。继承的 YOLO 绝不放松 `deny`（只读 worker 的 `write: deny` 仍然生效），上面的控制工具对它们也照常执行。继承是即时的：关闭 YOLO 后，SubAgent 的后续调用立即恢复确认。
...
对已有的普通文件，`write` 从不拒绝：它直接整体替换文件；若模型没有读取过当前版本（或文件在模型上次读取后发生了变化），写入前会把旧内容备份到会话目录，并在结果中提示文件已变化、给出备份位置，模型和用户看到的是同一段文本。变化发生在最近时，提示还会说明按文件修改时间看距离现在多久（例如「约 45s 前」），帮助模型区分「还有活跃的外部写入者」和「状态已稳定」；这个时间是尽力而为的，只在 24 小时内、且不早于 runtime 启动时间时才会显示（恢复的会话里，runtime 启动时间晚于会话本身的开始时间）。分页或被 budget 截断的 read 只会让旧内容按“未观察”处理并触发备份。若已有文件的内容完全无法读取（例如读权限被拒），写入仍会继续，只是无法备份，结果里只有提示、没有备份位置。磁盘内容与模型自己最近一次写入或编辑的结果一致时视为模型已知，直接覆盖，不提示也不备份。新文件创建不需要旧版本观察。`delete` 采用路径授权而非读取门控：其安全性由路径解析、权限规则、跟踪锁与删除前备份承担，删除文件不会强制先完整读取。正因为删除不要求先读取，除非模型在本会话中已经观察过该文件当前内容，否则删除前会尝试把文件的确切磁盘字节（包括空文件）备份到会话目录；备份不会跟随符号链接：删除链接只是移除目录项，目标文件不受影响；`write` 本身也拒绝跟随符号链接，因此链接目标永远不会被复制进会话目录。

`edit` 和 `apply_patch` 仍会在执行时读取当前磁盘，并通过 exact-match / patch-plan 校验防止陈旧定位；如果运行时检测到 drift 但当前锚点仍可验证，Chord 会提示警告而不是拒绝，并尽力把有风险的非空写前内容备份到当前会话目录。备份上限为每 path 10 个、每 session 200 个、单文件 10 MiB、每 session 总计 50 MiB；如果必须备份但超过这些上限或因其他原因失败，Chord 只记录日志，局部编辑仍会继续，模型结果不会增加备份失败诊断。删除/清理会话时，这些备份会随会话目录一起删除。
