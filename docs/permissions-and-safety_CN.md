...
权限可以在 Agent 配置中定义。推荐从下面这套个人开发模板开始，再按项目风险收紧或放宽：
...
这套配置的含义是：默认允许大多数工具；禁用 `Handoff` 与 `Delegate`；删除文件和常见高风险 shell/git 命令需要确认。权限规则按“最后匹配优先”生效，因此 `Bash` 下更具体的 `ask` 会覆盖顶层 `"*": allow`。它适合单人、可信工作区；共享仓库、团队服务或自动化 headless 部署应进一步收紧。
