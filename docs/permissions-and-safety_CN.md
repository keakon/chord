...
对已有文件，`write` 需要模型完整掌握当前版本：先完整读取，或当前内容正是模型自己此前的整文件写入；分页或被 budget 截断的 read 不足以授权整文件覆盖。若缺少完整观察基线，写入会在修改磁盘前拒绝并要求重新读取。只有文件发生了模型未知的改动（外部或其它 agent 的修改）时，写入才需要提示并备份：写入仍会继续并覆盖这些内容，而不要求模型再跑一次多余的读取；结果会提示文件已变化，并且只有之前非空内容的尽力备份确实成功时，才会给出备份位置及对应的源文件，模型和用户看到的是同一段文本。磁盘内容与模型自己最近一次写入或编辑的结果一致时视为模型已知，直接覆盖，不提示也不备份。新文件创建不需要旧版本观察。`delete` 采用路径授权而非读取门控：其安全性由路径解析、权限规则、跟踪锁与删除前备份承担，删除文件不会强制先完整读取。正因为删除不要求先读取，除非模型在本会话中已经观察过该文件当前内容，否则删除前会尝试把文件的确切磁盘字节（包括空文件）备份到会话目录；备份不会跟随符号链接：删除链接只是移除目录项，目标文件不受影响；`write` 本身也拒绝跟随符号链接，因此链接目标永远不会被复制进会话目录。
