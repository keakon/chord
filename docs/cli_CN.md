...
首次启动时，如果全局 `config.yaml` 缺失且 Chord 能取得控制 TTY，就会先启动一次性的初始化向导，再进入 TUI；它问什么、写哪些文件、没有控制 TTY 时怎么处理，见[快速开始](./quickstart_CN.md#2-第一次运行)。`help`、`version` 和非 root 子命令都不会触发这个向导。
