...
Chord 主要在 macOS 上开发和测试。其他平台在不同程度上可用：大部分功能与平台无关，少数功能依赖 OS 特定能力，在其他平台会降级或 no-op。本页给出每个功能的真实支持情况、降级行为，以及各平台需要额外安装什么。
...
[^im-linux]: `im-select` 是 macOS 起家的工具；Linux 上要么用兼容 build，要么自己写一个同 CLI 的 wrapper 脚本。
...
[^osc]: OSC 9 是终端的能力，不是 OS 的能力。iTerm2 等多数现代终端支持；旧 `xterm`、部分裸 tty、某些 `tmux` 配置下，转义会被静默丢弃。详见下文 [终端兼容](#终端兼容)。
[^clip]: 剪贴板图片粘贴依赖终端是否转发图片字节；并非所有终端都支持。iTerm2、新版 WezTerm/Ghostty 可以；某些 tmux/Linux 设置只能拿到路径或啥都没有。
[^img]: 图片渲染依赖终端的图片协议（iTerm2 inline images、Kitty graphics、Sixel 等）。终端都不支持时，图片附件依然可发给模型，但 TUI 内无预览。
[^lsp]: 需要本地装好对应 language server（如 `gopls`、`typescript-language-server`、`rust-analyzer`）。Chord 不打包它们。
[^py-unix]: Unix-like 系统下 Chord 在 LSP root 下依次探测 `.venv/bin/python`、`venv/bin/python`、`env/bin/python`。
...
macOS 下用 `caffeinate(1)` 实现。Linux / Windows / WSL 上是 no-op，实现是 `internal/power/power_other.go` 里的 `NoopBackend`。其他平台需要常驻不休眠的话，请直接用 OS 自带的电源设置。

### `ime_switch_target`

从 Insert 切到 Normal 时，Chord 可以调用 `im-select`（或 `im-select.exe`）切到指定 IM（通常是英文键盘布局），切回 Insert 时恢复原来的 IM。
...
`im-select` 需要单独安装。这个变量值就是个字符串，Chord 原样传给 `im-select`，具体格式由该平台工具决定。

### `desktop_notification`（OSC 9）

启用后，Chord 在权限确认、问题待回答、agent 回 idle 等事件时发出 OSC 9 转义序列。Chord 自己不做通知守护进程；具体显示依赖**终端**（iTerm2、WezTerm、Ghostty、kitty 等都支持）。

在 `tmux` 中可能需要 `set -g allow-passthrough on`，OSC 9 才能透传到宿主终端。

### 剪贴板图片粘贴

`Ctrl+V`（macOS 下 `Cmd+V`）优先粘贴剪贴板里的图片而不是文本。能否真正粘到取决于终端：
...
剪贴板图片粘贴不可用时，仍可用 `Ctrl+F` 把已写在输入框里的图片路径附加到消息。

### 终端图片渲染

Chord 自动检测 iTerm2 inline-images、Kitty graphics、Sixel。三者都不支持时，图片附件依然会发给模型，只是 TUI 内不预览。

### Pyright venv 自动探测

未在配置里手动指定 Python 解释器时，Chord 在 LSP root 下按以下顺序找项目 venv：
...
WSL 自动探测**故意不会**挑 `Scripts\python.exe` 里的 Windows venv。WSL 内开发请在 WSL 里建 Linux venv，或在 `lsp.pyright.options` 里显式设 `python.pythonPath`。

更多见 [扩展与定制 — LSP](./customization_CN.md#lsp)。

## 终端兼容

很多"在 macOS 上能用、Linux 不行"的反馈，实际是终端模拟器的差异，而不是 OS 差异。推荐使用以下终端获得最佳体验：
...
`tmux`、`screen` 在 Chord 与终端之间又加一层；部分功能（OSC 9、某些图片流程）需要显式配置 pass-through。
...
- 工具调用里的文件路径走 Windows 风格，反斜杠原样保留。
- 如果遇到 Windows 特有 bug，更可能是"还没人踩到"而不是"故意不支持"。请用 `Ctrl+G` 或 `/diagnostics` 导出诊断包再上报。
...
- Pyright venv 自动探测在 WSL 内使用 **Linux** venv（`.venv/bin/python` 等）；Windows 风格 `Scripts\python.exe` venv 故意不会被选。
