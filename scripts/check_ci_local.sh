...
# iterative verification stays fast; set CHORD_TEST_COUNT=1 for a cold test run.

min_coverage="${MIN_COVERAGE:-70.0}"
coverage_file="${COVERAGE_FILE:-coverage.out}"
test_count="${CHORD_TEST_COUNT:-}"
if [[ -z "${test_count}" && "${GITHUB_ACTIONS:-}" == "true" ]]; then
  test_count="1"
fi
stdout_file="$(mktemp)"
stderr_file="$(mktemp)"
...
leak_pattern='go: downloading|^\[[DIWE] [0-9]{4}-[0-9]{2}-[0-9]{2} |Device login URL|user_code:|Complete authorization|Login successful|Credentials written|client_fallback|API key permission denied'
if grep -En "${leak_pattern}" "${stdout_file}" "${stderr_file}" >/tmp/chord-ci-log-leaks.txt; then
  ...
  rm -f /tmp/chord-ci-log-leaks.txt
  exit 1
fi
rm -f /tmp/chord-ci-log-leaks.txt
