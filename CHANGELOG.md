...
- Provider-level timeout settings now allow per-provider overrides for total HTTP request timeout, stream idle timeout, and Responses WebSocket handshake timeout via `request_timeout`, `stream_idle_timeout`, and `websocket_handshake_timeout`.
