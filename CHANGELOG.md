...
- New opt-in `compat.reasoning_continuity.mode: anthropic_unsigned` for Messages-compatible endpoints (e.g. DeepSeek/GLM) that return visible unsigned thinking blocks: such thinking is replayed natively only to the same configured provider/model, while cross-provider or rejected replay degrades to marked assistant history that keeps the completed tool round. Visible OpenAI Chat `reasoning_content` is also converted into unsigned Anthropic thinking at the native compatibility level so cross-protocol fallback keeps the reasoning visible without fabricating signatures; a rejection degrades to the synthesized level without leaking reasoning into assistant text.
