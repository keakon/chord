...
- Provider retry pacing is now configurable with `retry_backoff: exponential | fixed | none` and `retry_delay_ms` (0–60000ms). It controls Chord-generated delays between complete model-pool rounds and, when explicitly configured, replaces `Retry-After` pacing for ordinary HTTP 429 key cooldown. Confirmed quota reset windows and hard credential states still take priority; without an explicit override, Chord honors the full valid `Retry-After`. As part of this change, every HTTP 429 now takes the ordinary rate-limit path: a Codex usage-limit 429 carrying neither a retry hint nor an exhausted quota snapshot starts at the shared one-second exponential default instead of the preset one-minute cooldown (the preset branch still covers non-429 usage-limit errors), and a 429 that interrupts visible streaming output now applies the same pacing and rotates to the next key instead of retrying the same key without a cooldown.
