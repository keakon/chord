...
- The `patch` tool is replaced by `apply_patch`, which speaks the Codex `*** Begin Patch` envelope and applies multi-file Add/Update/Delete/Move operations as one transaction: every operation is planned from a single filesystem snapshot, revalidated (content and file mode) before commit, and rolled back as a unit — including the file whose own write failed — when any commit step fails. Compatibility is preserved: the legacy `patch` tool name and single-file `{path, patch}` arguments keep working, and `patch` permission rule keys normalize to `apply_patch` at parse time. Permission patterns now match every path a patch touches, lexically cleaned so `./secret/x` cannot dodge a `secret/*` deny, and `*** Delete File:` operations still honor tool-specific `delete` rules. Deleting a file no longer requires a prior complete read — path resolution, permission rules, tracked locks, and pre-delete backups carry its safety instead.
