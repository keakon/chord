...
- The provider key `official_api` was split into `trust_http_400` (treat HTTP 400 as a terminal request error) and `retry_after_max_s` (longest honored `Retry-After` wait in seconds, 1–86400). Configs still containing `official_api` are rejected at startup by strict parsing. Migrate `official_api: true` to `trust_http_400: true` plus `retry_after_max_s: 86400`; migrate `official_api: false` to `trust_http_400: false` or omit it. Third-party providers now cap `Retry-After` at 60 seconds by default, so set `retry_after_max_s: 86400` explicitly if the old configuration relied on longer hints. `preset: codex` infers the trusted-400 and one-day cap settings automatically.
...
- The `apply_patch` tool card now names what each section is: the planned file list is `Targets:`, the committed diff is `Applied changes:`, and the submitted input is `Requested patch:`. A partially applied patch separates its applied summary, `Error:` text, and `Diagnostics:` into ordered sections instead of running them together, and stops repeating the requested patch that the diff above already shows. Target, operation, and patch lines are truncated with `…` rather than wrapped, so one file or one patch line stays on one row and the card structure survives a narrow terminal; copying the card (`yy`) still yields the untruncated text. Long diff lines elided in the middle now keep the diff row background across the `…` marker instead of punching a hole in the colored band.
...
- Plaintext `reasoning_content` and unsigned thinking blocks from completed turns are no longer replayed to the backend by default. Most thinking-mode backends validate reasoning only after the last user message and drop earlier-turn reasoning server-side, so the replayed history was billed on every request and never used — over half of a large request on the affected endpoints. Current-turn reasoning and signed or redacted Anthropic thinking blocks are untouched. Preserved-thinking models whose documentation requires the complete assistant history (Kimi K3 / `keep: all`, Qwen `preserve_thinking`, GLM `clear_thinking: false`) keep it via the new `compat.reasoning_continuity.preserve_history: true`; the recipes in the model configuration guide set it accordingly.
- OpenAI-compatible endpoints that reject forced `tool_choice: required` while reasoning is active can now opt out per model with `compat.forced_tool_choice.suppress_in_thinking: true`. The DeepSeek V4 Chat recipe enables it, while the Responses recipe keeps `required` because that endpoint supports it. Suppression follows the final request overrides, so an override that disables reasoning does not downgrade tool choice unnecessarily.
...
- Verification declarations are no longer rejected on epoch gaps: the newest declared command must cover the current workspace epoch, while earlier declared commands may come from earlier epochs. This keeps stale verification blocked without making honest multi-command declarations such as lint plus test impossible.
