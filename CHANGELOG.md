...
- Background `spawn` completions now always leave a result card in the main transcript. Cards for the main agent's own background work carried an agent attribution the main view filters out, so only the toast notification was visible; completions arriving while Chord was idle emitted no card at all before the follow-up reply; and the card could still be dropped when the UI event channel was saturated. Results whose owning SubAgent no longer exists are now shown in the main transcript instead of a view that can no longer be opened.
