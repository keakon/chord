...
- Fixed TUI cards, local shell output, question dialogs, and confirmation summaries so ANSI-rich tool/file output is displayed literally instead of being re-executed as terminal styling. This now includes bare carriage-return progress/control text as well, preventing rendering corruption when viewing diagnostic dumps, shell output, or other external text containing escape/control sequences.
