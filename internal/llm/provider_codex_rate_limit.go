...
func (p *ProviderConfig) persistCodexResetHintsForKey(key string, primaryResetAt, secondaryResetAt int64) bool {
	...
	match := config.OAuthCredentialMatch{AccountUserID: ks.OAuthInfo.AccountUserID, AccountID: ks.OAuthInfo.AccountID, Email: ks.OAuthInfo.Email, Access: ks.Key, CredentialIndex: &credentialIndex}
