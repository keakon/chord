...
func TestDurationFromPositiveMillisecondsClamped(t *testing.T) {
	if got := durationFromPositiveMillisecondsClamped(0, 0); got != 0 {
		t.Fatalf("durationFromPositiveMillisecondsClamped(0, 0) = %v, want 0", got)
	}
	if got := durationFromPositiveMillisecondsClamped(120_000, time.Minute); got != time.Minute {
		t.Fatalf("durationFromPositiveMillisecondsClamped(120000, 1m) = %v, want 1m", got)
	}
	if got := durationFromPositiveMillisecondsClamped(maxSafeDurationMilliseconds+1, 0); got != maxTimeDuration {
		t.Fatalf("durationFromPositiveMillisecondsClamped(max+1, 0) = %v, want %v", got, maxTimeDuration)
	}
}
