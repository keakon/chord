...
func ParsePatch(path, patchText string) (parsedPatch, error) {
	...
	parsed, err := parsePatchStrict(cleanPath, patchText)
	if err == nil {
		return parsed, nil
	}
	if strippedPatchText, ok := stripTrailingMalformedEndPatch(patchText); ok {
		if parsed, retryErr := parsePatchStrict(cleanPath, strippedPatchText); retryErr == nil {
			return parsed, nil
		}
	}
	return parsedPatch{}, err
...
func parsePatchStrict(cleanPath, patchText string) (parsedPatch, error) {
	...
	for _, line := range lines {
		...
		default:
			...
			kind := line[0]
			if kind != ' ' && kind != '+' && kind != '-' {
				return parsedPatch{}, invalidPatchLineError(line)
			}
...
func stripTrailingMalformedEndPatch(patchText string) (string, bool) {
	lines := splitPatchLines(strings.ReplaceAll(patchText, "\r\n", "\n"))
	last := len(lines) - 1
	for last >= 0 && strings.TrimSpace(lines[last]) == "" {
		last--
	}
	if last < 0 || !strings.HasPrefix(strings.TrimSuffix(lines[last], "\r"), "*** End Patch") {
		return patchText, false
	}
	cleaned := append([]string{}, lines[:last]...)
	cleaned = append(cleaned, lines[last+1:]...)
...
func invalidPatchLineError(line string) error {
	return fmt.Errorf("invalid patch line %q: every non-empty hunk line must start with a patch marker: space for unchanged context, + for additions, or - for deletions. The context marker space is separate from source indentation; preserve the source line's own indentation after that marker. For example, if the unchanged source line is `    value` with four leading spaces, write it in the hunk as `     value`: one context marker space plus the original four indentation spaces. No files were modified", line)
}

func hunkHasChanges(h patchHunk) bool {
