...
func (CompactContextTool) Description() string {
	return "Request a durable context checkpoint once your current working state is fully externalized (written into state_files or fully expressible in structured arguments).\n" +
		...
		"Roles that are allowed to write plan or notes files (for example .chord/plans/plan-*.md or a task-notes file under .chord/notes/ in a planner role) may list those files here; state_files itself never reads or writes anything, and write permissions are still governed by the role's permission rules.\n" +
