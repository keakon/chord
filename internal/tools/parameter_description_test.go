...
func TestToolParameterDescriptionsMentionDefaults(t *testing.T) {
	tests := []struct {
		...
		{name: "shell yield", tool: NewShellTool(""), path: []string{"yield_time_ms"}, want: "default 90000"},
		{name: "job output wait", tool: JobOutputTool{}, path: []string{"wait"}, want: "`none` (default) returns immediately"},
		{name: "web fetch raw", tool: WebFetchTool{}, path: []string{"raw"}, want: "Default false."},
		{name: "web fetch timeout", tool: WebFetchTool{}, path: []string{"timeout_ms"}, want: "default 30000"},
