...
func TestCompactionStatusBarRightCacheAdvancesWithTimeFrame(t *testing.T) {
	...
	first, _, _ := m.renderStatusBarRightSide(now, 120, 0, 0, statusBarPathForms{}, "", 0, 0)
	firstKey := m.cachedStatusBarRightKey
	second, _, _ := m.renderStatusBarRightSide(now.Add(2*time.Second), 120, 0, 0, statusBarPathForms{}, "", 0, 0)
...
func TestCompactionPillSurvivesBusyActivityLane(t *testing.T) {
	...
	rightSide, rightStart, _ := m.renderStatusBarRightSide(now, effectiveWidth, leftWidth, activityWidth, statusBarPathForms{value: "/Users/keakon/Workspace/chord"}, "20260902024540990", 0, 0)
