...
func (m *Model) handleInsertKey(msg tea.KeyMsg) tea.Cmd {
	...
	case keyMatches(key, m.keyMap.InsertSubmit):
		...
		if m.input.BangMode() {
			...
			m.nextBlockID++
			userBlock := &Block{ID: shellID, Type: BlockUser, Content: userLine, AgentID: m.focusedAgentID, ImageCount: 0, Collapsed: false, UserLocalShell: true, UserLocalShellCmd: cmdStr, UserLocalShellPending: strings.TrimSpace(cmdStr) != "", MsgIndex: -1}
