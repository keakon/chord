...
`)
	...
	for _, p := range []string{"secret/plan.txt", filepath.Join(pathTestCWD, "secret/plan.txt")} {
		...
		got := evaluateToolPermissionInDir(ruleset, "delete", args, pathTestCWD)
...
`)
	...
	if got := evaluateToolPermissionInDir(ruleset, "delete", inCWD, pathTestCWD); got.Action != permission.ActionDeny {
...
`)
	...
	if got := evaluateToolPermissionInDir(ruleset, "read", inCWD, pathTestCWD); got.Action != permission.ActionAllow {
		t.Fatalf("in-cwd read = %#v, want allow under '**'", got)
	}
	outCWD := json.RawMessage(`{"path":"/Users/me/other/foo.go"}`)
	if got := evaluateToolPermissionInDir(ruleset, "read", outCWD, pathTestCWD); got.Action != permission.ActionDeny {
...
`)
	...
	inDir := evaluateToolPermissionInDir(ruleset, "delete", args, "")
...
`)
	...
	for name, tc := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			got := evaluateToolPermissionInDir(ruleset, tools.NameApplyPatch, args, pathTestCWD)
...
`)
	...
	if got := evaluateToolPermissionInDir(ruleset, "write", writeIn, pathTestCWD); got.Action != permission.ActionAllow {
		t.Fatalf("in-cwd write = %#v, want allow under 'src/**'", got)
	}
	writeOut := json.RawMessage(`{"path":"/Users/me/other/gen.go","content":"package gen"}`)
	if got := evaluateToolPermissionInDir(ruleset, "write", writeOut, pathTestCWD); got.Action != permission.ActionDeny {
	...
	if got := evaluateToolPermissionInDir(ruleset, "edit", editIn, pathTestCWD); got.Action != permission.ActionAllow {
	...
	if got := evaluateToolPermissionInDir(ruleset, "edit", editOut, pathTestCWD); got.Action != permission.ActionDeny {
