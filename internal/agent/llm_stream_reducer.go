...
type llmStreamReducer struct {
	...
	onRetryError     func(error, string, string, string, string, string)
...
func (r *llmStreamReducer) Handle(delta message.StreamDelta) {
	...
	case message.StreamDeltaRetryError:
		if r.onRetryError != nil {
			r.onRetryError(delta.Err, delta.Provider, delta.Model, delta.MaskedKey, delta.AccountID, delta.Email)
		}
