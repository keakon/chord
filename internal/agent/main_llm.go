...
func (a *MainAgent) callLLM(ctx context.Context, messages []message.Message) (*message.Response, error) {
	...
	if log.IsEnabledFor(golog.DebugLevel) {
		...
		a.llmMu.RUnlock()
		log.Debugf("LLM request context contributors messages=%v estimated_tokens=%v reduction_messages=%v reduction_bytes=%v reduction_tokens_before=%v reduction_tokens_after=%v reduction_tokens_saved=%v reduction_saved_delta=%v reduction_protected=%v reduction_protect_reason=%v reduction_reused_stable=%v reduction_reuse_reason=%v previous_model=%v model_changed=%v model_run_length=%v reduction_by_tool_rule=%v reduction_skipped_by_reason=%v reduction_over_compression=%v reduction_by_retention_level=%v reduction_protected_read_tokens=%v reduction_unrecoverable=%v reduction_reference_pinned=%v archive_reads=%v archive_read_failures=%v evidence_rebuild_us=%v evidence_files=%v evidence_observations=%v evidence_current=%v evidence_stale=%v evidence_superseded=%v top=%v", len(messages), llm.EstimateRequestInputTokens(installedPrompt, messages, toolDefs), stats.Messages, stats.Bytes, stats.TokensBefore, stats.TokensAfter, stats.TokensSaved, stats.SavedDelta, stats.Protected, stats.ProtectReason, stats.ReusedStable, stats.ReuseReason, stats.PreviousModel, stats.ModelChanged, stats.ModelRunLength, stats.ByToolAndRule, stats.SkippedByReason, stats.OverCompression, stats.ByRetentionLevel, stats.ProtectedReadTokens, stats.UnrecoverableReductions, stats.ReferencePinned, stats.ArchiveReads, stats.ArchiveReadFailures, stats.EvidenceRebuildDurationUS, stats.EvidenceFiles, stats.EvidenceObservations, stats.EvidenceCurrent, stats.EvidenceStale, stats.EvidenceSuperseded, labels)
	}
