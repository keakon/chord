...
func (a *MainAgent) modelDrivenContextPromptBlock() string {
	...
	return "## Long-session context management\n" +
		...
		"- Files you maintain and are allowed to write in this role (for example plan documents under .chord/plans/ named YYYYMMDD-<slug>.md or task notes under .chord/notes/) can be listed in the compact_context state_files parameter when requesting a checkpoint: state_files entries are pure references (the runtime never probes them for existence), and after the checkpoint applies it re-loads a bounded head of every listed file this session already read or wrote while the read permission rule allows; read them with the read tool when you need more than that head, instead of scanning the archived history files.\n" +
		"- Read the registered files first after a reset, keep the top of each a self-contained resume block (objective, next step, open issues), and list only files you created or updated in this session: a path you never wrote is usually stale and every listed file costs a re-read. Leave state_files empty only when no durable file exists yet (a pure analysis or final-report stage) or this role cannot write files.\n" +
